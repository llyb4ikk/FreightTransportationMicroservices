...
namespace FreightTransport.Controllers
{
    ...
    public class TransportationController : Controller
    {
        ...
        [HttpPut]
        [Route("SetDriversToTransportyation/{transportationId}")]
        public async Task<IActionResult> SetDriversToTransportyation([FromBody] IEnumerable<int> carDriverIds, int transportationId)
        {
            var result = await _service.SetDriversToTransportyation(transportationId, carDriverIds);
            if (result)
                return Ok(result);
            return NotFound();
        }
